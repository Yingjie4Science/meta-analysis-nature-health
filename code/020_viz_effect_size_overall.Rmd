...
readr::write_csv(x = ma_result_all, file = './data/0302-MA-output/ma_result_all.csv')
saveRDS(ma_result_all, file = './data/0302-MA-output/ma_result_all.RDS')
```
...
if(vv == "vPr") {
  ...
  data <- ma_result_all %>%
    dplyr::filter(!tool %in% c('STAI', 'DASS'))
}
...
```{r - demo run, eval=FALSE, include=FALSE}
...
str(ma_result_each_i)
subgroup_unique <- unique(ma_result_each_i$subgroup) %>% sort()

ma_result_each_i <- ma_result_each_i %>%
  mutate(subgroup = factor(x = subgroup, levels = subgroup_unique))
...
#   dplyr::filter(subgroup == 'stay static')
# 
# ma_result_each_i2 <- ma_result_each_i %>%
#   dplyr::filter(subgroup == 'physical activity')
# 
...
# fname <- paste0(dir.fig, 'es_comb_subgroup_', subgroup_select, '_TestDiff_', today, vv, '_cld.png'); print(fname)
# func_ggsave(fname, w = 7, h = 6, save_png = T)



# source('code/func_comparing_means_Dunns.R')
# func_test_dif_dunn(df = ma_result_each_i, value = 'SMD', group = 'subgroup', facet_by = 'ind_sub')
# fname <- paste0(dir.fig, 'es_comb_subgroup_', subgroup_select, '_TestDiff_', today, vv, '_sig.png'); print(fname)
# func_ggsave(fname, w = 7, h = 6, save_png = T)



source('code/func_comparing_means_Dunns.R')
func_test_dif_dunn2(df = ma_result_each_i, value = 'SMD', group = 'subgroup', facet_by = 'ind_sub')
fname <- paste0(dir.fig, 'es_comb_subgroup_', subgroup_select, '_TestDiff_', today, vv, '_sig2.png'); print(fname)
func_ggsave(fname, w = 7, h = 6, save_png = T)
...
for (subgroup_select in subgroups) {
  ...
  func_test_dif_dunn2(df = ma_result_each_i, value = 'SMD', group = 'subgroup', 
                      ...
                      facet_by = 'ind_sub')
  fname <- paste0(dir.fig, paste('es_comb_subgroup', subgroup_select, 'TestDiff', today, which_test, vv, sep = '_'), '.png'); print(fname)
  func_ggsave(fname, w = 7, h = 6, save_png = T)
}
