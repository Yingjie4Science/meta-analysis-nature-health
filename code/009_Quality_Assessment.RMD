...
csv <- "review_278532_20250218115356.csv" ## 2025-02-17, R1
...
# ## qa + all other data
# f <- paste0(dir.raw, 'review_278532_20250227113241.csv')
...
q1 <- readr::read_csv(file = f, show_col_types = F) %>%
  dplyr::mutate(date = t) %>%
  dplyr::rename(
    'id' = 'Covidence #',
  ...
  dplyr::select(!matches("Model ")) %>%
  arrange(id, Reviewer) %>%
  as.data.frame()



## or read data from the pre-cleaned dataset -- filter out rows without effect size data
# f <- paste0("./data/0302-MA-output/", "df_cleaned.RDS")
# q2 <- readRDS(f) %>%
#   dplyr::select(any_of(names(q1)), Tools)


## this raw data include rows without effect size
f <- './data/included_papers_for_systematic_review_rawdata.csv'
q2 <- readr:::read_csv(f, show_col_types = F) %>%
  dplyr::select(!matches("supporting text|^---")) %>%
  dplyr::select(!matches("Model ")) %>%
  # dplyr::select(any_of(names(q1)), Tools) %>%
  dplyr::select(!matches("Description of|^---")) %>%
  dplyr::select(any_of(names(q1)), `Study design`, Tools) %>%
  dplyr::mutate(id = as.character(id)) %>%
  as.data.frame()


## which data to use for further analysis?
q <- q1 %>%
  dplyr::left_join(x = q1, 
            y = q2 %>% select(id, Tools), 
            by = 'id')


## to add data
q2_study_design_add <- q2 %>%
  filter(is.na(`Study design`))
```
...
gs_all <- gs %>%
  dplyr::filter(!exclude %in% c('yes', 'TBD')) %>%
  # Desired tools to sort at the top
  dplyr::mutate(rank = case_when(
    Tool %in% tool_selected_rct ~ 1,
    TRUE ~ 2
  )) %>%
  arrange(rank, Tool) %>%
  ...
  dplyr::mutate(design_class = case_when(
    `study design` %in% c('Cross sectional study', 'Observational') ~ 'Observational',
    str_detect(`study design`, 'Longitudinal') ~ 'Observational',
    T ~ "Experimental"
  )) %>%
  dplyr::select(1:`study design`, design_class, everything()) %>%
  as.data.frame()


gs_exp <- gs_all %>%
  dplyr::filter(design_class == 'Experimental') 

## save a copy 
f <- './data/0302-MA-output/gsheet_qa_experimental_studies.CVS'
readr::write_csv(x = gs_exp %>% select(1:3, Tool, `study design`), file = f)
...
gs_all_stat <- gs_all %>%
  group_by(`study design`, design_class) %>%
  tally() %>%
  ungroup() %>%
  dplyr::mutate(perc = n / sum(n, na.rm = T)) %>% 
  arrange(perc) %>%
  dplyr::mutate(labels = scales::percent(perc, accuracy = 0.1))

n_total <- sum(gs_all_stat$n, na.rm = T)


p1 <- gs_all_stat %>%
  ggplot(aes(x = reorder(`study design`, perc), y = n, fill = design_class)) + 
  geom_col() +
  geom_text(aes(label = n), vjust = 0.5, hjust = -.3) +
  xlab('')+
  labs(fill='') +
  coord_flip() +
  theme(legend.position = c(0.8, 0.2), legend.key.size = unit(0.15, 'in'),
...
qa_exp_data <- qa %>%
  ...
  dplyr::select(-any_of(c('Tools')))


qa_exp_data_l <- qa_exp_data %>%
  pivot_longer(cols = 5:ncol(.), names_to = 'Item') %>%
  dplyr::mutate(
    value = stringr::str_squish(value), 
    rate = dplyr::case_when(
      value == 'High risk of bias' ~ 3,
      value == 'Unclear'  ~ 2,
      value == 'Low risk of bias' ~ 1,
      TRUE ~ NA )) %>%
  group_by(id, `Study ID`) %>%
  dplyr::summarise_at(c("rate"), list(mean = ~mean(.), sum = ~sum(.)), na.rm = TRUE) %>%
  dplyr::mutate(
    bias_level = case_when(
      #' as two items are about blinding, which is not avoidable, we only count on the remaining five = (7-2).
      #' if more than half are high bias, i.e., 5/2*3
      mean > 2 ~ '3 - high bias',
      T ~ '1 - low bias'
    )) %>%
  as.data.frame()
