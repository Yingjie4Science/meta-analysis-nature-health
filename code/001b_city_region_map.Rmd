...
conflicts_prefer(cowplot::theme_nothing)
...
```{r - region - bar, paged.print=FALSE}

reg_id <- dt %>%
  dplyr::select(1, Country) %>%
  dplyr::filter(!is.na(Country)) %>%
  dplyr::mutate(Country = case_when(
    Country %in% c('m', 'M', 'NA', 'Global', 'Europe', 'all over the world') ~ 'TBD',
    TRUE ~ Country)) %>%
...
region_iso3c_id <- reg_id %>%
  ...
  dplyr::distinct(id, iso3c, Country, .keep_all = T) %>%
  arrange(id) %>%
  as.data.frame()
...
pub_byregion_map <- region_count_sf %>%
  ...
  # theme_bw() +
  theme_nothing() +
  # guides(fill=guide_legend(title="Number \nof papers\nby country", theme = theme(legend.title = element_text(size = 7))),
  #        size=guide_legend(title="Number \nof papers\nby city",    theme = theme(legend.title = element_text(size = 7)))) +
...
```{r - Combined plot - by year & region map}
w_map <- 1.2
plot.with.inset <-
  ggdraw() +
...
```{r - map - cities in China}

pub_byregion_map_cn <- region_count_sf %>%
  dplyr::filter(name != 'Antarctica') %>%
  dplyr::filter(iso_a3 == 'CHN') 


# Step 2: Define Albers Equal-Area Conic projection (specific for China)
crs_albers <- "+proj=aea +lat_1=25 +lat_2=47 +lon_0=105 +datum=WGS84 +units=m +no_defs"
pub_byregion_map_cn_albers <- sf::st_transform(
  pub_byregion_map_cn, 
  crs = crs_albers)


dt_city_count_cn <- sf::st_as_sf(dt_city_count, coords = c("lon", "lat"), crs = 4326) %>%
  dplyr::filter(Country %in% c('China', 'Taiwan'));
dt_city_count_cn_albers <- sf::st_transform(dt_city_count_cn, crs = crs_albers)


# Step 3: Plot the reprojected map using ggplot2
p <- 
  pub_byregion_map_cn_albers %>%
  ggplot(.) +
  geom_sf(fill = 'gray95', size = 0.1, color = 'gray40') +

  
  geom_sf(data = dt_city_count_cn_albers,
          aes(size = count), 
          color = 'red', shape = 1, alpha = 0.5) +
  # geom_point(data = dt_city_count , 
  #            aes(x = lon, y = lat, 
  #                size = count), color = 'red', shape = 16, alpha = 0.5) +
  scale_size_continuous(name = 'Study #', range = c(0.8, 5), breaks = c(1, 2, 4, 6, 8)) +
...
fname <- paste0(dir.fig, 'pub_byYearRegion_map_cn_', today, '_v2.png'); fname
func_ggsave(fname = fname, w = 6.5, h = 5, dpi = 300, save_png = T)
```
